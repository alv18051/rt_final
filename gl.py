...
class Raytracer(object):
    ...
    def cast_ray(self, orig, dir, sceneObj = None, recursion = 0):
        ...
        if material.matType == OPAQUE:
            ...
            outside = mm.producto_punto(dir, intersect.normal) < 0
            bias = 0.001 * intersect.normal

            specColor = np.array([0,0,0])
            for light in self.lights:
                specColor = mm.suma_vector(specColor, light.getSpecColor(intersect, self))

            reflect = reflectVector(intersect.normal, np.array(dir) * -1)
            
            reflectOrig = mm.suma_vector(intersect.point, bias) if outside else np.subtract(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrig = mm.resta_vector(intersect.point, bias) if outside else mm.suma_vector(intersect.point, bias)
